...
function applyFilters() {
  ...
  document.querySelectorAll("#productTable tbody tr[data-id]").forEach((tr) => {
    const show =
      (!kw || tr.dataset.name.includes(kw)) &&
      (!cat || tr.dataset.cat === cat || tr.dataset.parentCat === cat) &&
      (!status || tr.dataset.status === status);
    tr.style.display = show ? "" : "none";
  });
}
[searchInput, filterCat, filterStatus].forEach((el) =>
