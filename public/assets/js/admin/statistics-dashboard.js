...
function updateMonthlyTable(data) {
  ...
  months.forEach((month) => {
    ...
    const trendColor =
      trend >= 70 ? "#22c55e" : trend >= 40 ? "#f59e0b" : "#ef4444";
...
async function updateAllCharts() {
  ...
  try {
    const response = await fetch(
      `/WEB_GR4/admin/api/statistics/stats?from=${from}&to=${to}`,
    );
  ...
  try {
    ...
    const monthlyResponse = await fetch(
      `/WEB_GR4/admin/api/statistics/monthly-chart`,
    );
    ...
    const topProductsResponse = await fetch(
      `/WEB_GR4/admin/api/statistics/top-products?from=${from}&to=${to}`,
    );
    ...
    const orderStatusResponse = await fetch(
      "/WEB_GR4/admin/api/statistics/order-status",
    );
...
function updateStatCards(stats) {
  ...
  const bestsellerRevenue = document.querySelectorAll(
    ".featured-stat .value",
  )[1];

  if (bestsellerName && stats.best_seller) {
    bestsellerName.textContent =
      stats.best_seller.product_name || "Chưa có dữ liệu";
  }
  if (bestsellerQty && stats.best_seller) {
    bestsellerQty.textContent = (
      stats.best_seller.total_sold || 0
    ).toLocaleString();
  }
  if (bestsellerRevenue && stats.best_seller) {
    bestsellerRevenue.textContent = formatCurrency(
      stats.best_seller.revenue || 0,
    );
  }
}
